...
FastAPI + JWT oturum
Çalıştır:
    uvicorn api_unified:app --reload --host 0.0.0.0 --port 8000
"""
...
app = FastAPI(title="LOGLine WMS - Mobile + Web API", version="2.0.0")
...
# ═══════════════════════════════════════════════════════════════════
# WEB DASHBOARD API ENDPOINTS - React Frontend İçin
# ═══════════════════════════════════════════════════════════════════
...
# Web static files mount - production build için
try:
    app.mount("/static", StaticFiles(directory="web/frontend/build/static"), name="static")
    print("Web static files mounted successfully")
except RuntimeError:
    print("Web static files not found - development mode")

@app.get("/web/api/dashboard/stats")
async def web_dashboard_stats(_: TokenData):
    """Web dashboard istatistikleri - mobil API'den farklı endpoint"""
    conn, cur = get_conn_cur()
    
    try:
        stats = {}
        
        # 1. Toplam Siparişler - LOGO tablosundan
        cur.execute("SELECT COUNT(*) as cnt FROM LG_025_01_ORFICHE WHERE STATUS IN (1, 2, 3) AND TRCODE = 1")
        orders_result = cur.fetchone()
        stats["orders"] = orders_result.cnt if orders_result else 0
        
        # 2. Stok Kalemleri - LOGO items tablosundan  
        cur.execute("SELECT COUNT(*) as cnt FROM LG_025_ITEMS WHERE ACTIVE = 0 AND CARDTYPE = 1")
        items_result = cur.fetchone()
        stats["items"] = items_result.cnt if items_result else 0
        
        # 3. Aktif Kullanıcılar (fallback)
        stats["users"] = 5
        
        # 4. Bugün Aktivite (fallback)
        stats["activities"] = 12
        
        # 5. Sistem Uyarıları (fallback) 
        stats["alerts"] = 2
        
        # 6. Ambar İşlemleri - LOGO stok fişlerinden
        cur.execute("SELECT COUNT(*) as cnt FROM LG_025_01_STFICHE WHERE CAST(DATE_ AS DATE) = CAST(GETDATE() AS DATE) AND TRCODE IN (1, 2, 3, 4)")
        ...
        print(f"Web dashboard stats error: {e}")
        return {
            "orders": 0, "items": 0, "users": 5,
            "activities": 12, "alerts": 2, "warehouse": 0,
            "error": str(e)
        }
    finally:
        conn.close()

@app.get("/web/api/system/status") 
async def web_system_status(_: TokenData):
    """Web sistem durumu"""
    try:
        conn, cur = get_conn_cur()
        ...
        # LOGO database test
        cur.execute("SELECT GETDATE() as current_time")
        logo_test = cur.fetchone()
        ...
        print(f"Web system status error: {e}")
        return {
...
async def serve_web_app():
    ...
    <head>
        ...
        <style>
            ...
            .links a { display: inline-block; background: #3498db; color: white; padding: 10px 20px; margin: 5px; text-decoration: none; border-radius: 5px; }
            .links a:hover { background: #2980b9; }
        </style>
    </head>
    <body>
        <div class="container">
            <h1 class="title">LOGLine Web Dashboard</h1>
            <div class="status">Web uygulamasi basariyla calisiyor!</div>
            <div class="status">Mobil API ayni portta korunuyor</div>
            
            <div class="links">
                <a href="/web/api/dashboard/stats">Dashboard API</a>
                <a href="/web/api/system/status">System Status</a>
                <a href="/docs">API Docs</a>
            </div>
            
            <p><strong>Port 8000:</strong> Mobil + Web unified API</p>
            <p><strong>Mobil endpoints:</strong> /login, /trips, /scan/qr vb.</p>
            <p><strong>Web endpoints:</strong> /web/api/* altında</p>
        </div>
    </body>
    </html>
    """)